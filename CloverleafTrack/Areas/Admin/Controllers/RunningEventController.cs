...
[Area("Admin")]
[Route("admin/runningevent")]
public class RunningEventController : Controller
{
    private readonly IRunningEventService eventService;

    public RunningEventController(IRunningEventService eventService)
    {
    ...
    public IActionResult Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
    {
        ...
        return View(PaginatedList<RunningEvent>.Create(events, pageNumber ?? 1, pageSize));
    }
    ...
    public async Task<IActionResult> Create([Bind("Name,Gender,SortOrder,Environment")] RunningEvent @event, CancellationToken token)
    {
        try
        {
    ...
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> EditPost(Guid? id, [Bind("Id,Name,Gender,SortOrder,Environment")] RunningEvent @event, CancellationToken token)
    {
