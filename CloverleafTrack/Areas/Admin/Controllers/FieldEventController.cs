...
public class FieldEventController : Controller
{
    ...
    public IActionResult Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
    {
        ...
        switch (sortOrder)
        {
            ...
            case "deleted_ascending":
                events = events.OrderBy(x => x.Deleted).ToList();
                break;
            case "deleted_descending":
                events = events.OrderByDescending(x => x.Deleted).ToList();
                break;
            default:
        ...
        return View(PaginatedList<FieldEvent>.Create(events, pageNumber ?? 1, pageSize));
    }
    ...
    public async Task<IActionResult> Create([Bind("Name,Gender,SortOrder,Environment")] FieldEvent @event, CancellationToken token)
    {
        try
        {
    ...
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> EditPost(Guid? id, [Bind("Id,Name,Gender,SortOrder,Environment")] FieldEvent @event, CancellationToken token)
    {
